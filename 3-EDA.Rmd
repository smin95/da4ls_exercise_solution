...
<div class="question">
Download and load **by typing** `load(skew.RData)` into R. Use exploratory data analysis tools to determine which two columns are different from the rest. Which column has
...
t.test(x_add,y) # t-test after outlier
```
...
```{r}
wilcox.test(x_add,y) # after outlier
```
